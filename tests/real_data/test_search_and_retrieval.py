...
class TestSearchAndRetrievalRealData:
    ...
    async def test_search_multiple_types(self):
        ...
        logger.info(f"Search for '{TEST_SEARCH_QUERY}' returned {len(results)} results")
        logger.debug(f"Result types found: {result_types}")
    ...
    async def test_search_artists_only(self):
        ...
        logger.info(f"Found {len(artist_results)} artists for '{TEST_ARTIST_NAME}':")
        for i, artist in enumerate(artist_results[:5]):
            logger.debug(f"  {i + 1}. {artist.name} (ID: {artist.id})")
            ...
            if hasattr(artist, 'tags') and artist.tags:
                logger.debug(f"     Tags: {', '.join(artist.tags[:3])}")
    ...
    async def test_search_albums_only(self):
        ...
        logger.info(f"Found {len(album_results)} albums for '{TEST_ALBUM_NAME}':")
        for i, album in enumerate(album_results[:5]):
            logger.debug(
                f"  {i + 1}. {album.name} by {album.artist_name} (ID: {album.id})"
            )
            if hasattr(album, 'artist_name'):
                logger.debug(f"     Artist: {album.artist_name}")
    ...
    async def test_search_tracks_only(self):
        ...
        logger.info(f"Found {len(track_results)} tracks for '{TEST_TRACK_NAME}':")
        for i, track in enumerate(track_results[:5]):
            logger.debug(
                f"  {i + 1}. {track.name} by {track.artist_name} (ID: {track.id})"
            )
            if hasattr(track, 'album_name') and track.album_name:
                logger.debug(f"     Album: {track.album_name}")
    ...
    async def test_search_case_sensitivity(self):
        ...
        logger.info(f"Lowercase search: {len(lower_results)} results")
        logger.info(f"Uppercase search: {len(upper_results)} results")
    ...
    async def test_search_partial_matches(self):
        ...
        logger.info(f"Partial search for 'Path': {len(partial_results)} results")

        # Should find Pathos
        if len(partial_results) > 0:
            found_pathos = any("pathos" in r.name.lower() for r in partial_results)
            logger.info(f"Found Pathos in partial results: {found_pathos}")
            ...
            if found_pathos:
                ...
                logger.debug(
                    f"Pathos match: {pathos_result.name} (ID: {pathos_result.id})"
                )
        else:
            logger.info("No results for partial search")
    ...
    async def test_search_special_characters(self):
        ...
        logger.info(
            f"Special character search '{special_query}': {len(results)} results"
        )
        assert isinstance(results, list), "Search with special chars should return list"

        if len(results) > 0:
            for i, result in enumerate(results[:3]):
                logger.debug(f"  {i + 1}. {result.name} ({result.type})")
    ...
    async def test_search_unicode_characters(self):
        ...
        logger.info(f"Unicode search '{unicode_query}': {len(results)} results")
    ...
    async def test_search_result_consistency(self):
        ...
        logger.info(f"First search: {len(results1)} results")
        logger.info(f"Second search: {len(results2)} results")
        ...
        logger.info(f"Result overlap: {overlap} / {min(len(results1), len(results2))}")
    ...
    async def test_search_data_structure_validation(self):
        ...
        if len(results) == 0:
            logger.info("No search results to validate")
            return

        # Take first result for detailed validation
        result = results[0]

        logger.info(f"Validating search result: {result.name}")
        ...
        if result.type == "artist":
            ...
            logger.debug(f"  Location: {getattr(result, 'location', 'Unknown')}")
            logger.debug(f"  Is label: {getattr(result, 'is_label', False)}")
            logger.debug(f"  Tags: {getattr(result, 'tags', [])}")
            ...
            logger.debug(f"  Artist ID: {getattr(result, 'artist_id', 'Unknown')}")
            logger.debug(f"  Artist name: {getattr(result, 'artist_name', 'Unknown')}")
            ...
            logger.debug(f"  Artist ID: {getattr(result, 'artist_id', 'Unknown')}")
            logger.debug(f"  Artist name: {getattr(result, 'artist_name', 'Unknown')}")
            logger.debug(f"  Album name: {getattr(result, 'album_name', 'Unknown')}")
